...
class MasterProvinsiController extends Controller
{
    ...
    {
        return view('admin.master-data.master-provinsi',[
            'title' => $this->title,
        ]);
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'nama' => 'required|unique:master_provinsi',
            'deskripsi' => 'nullable',
        ],
        [
        ...
        if ($validator->fails()) {
            ...
            try {
                DB::beginTransaction();
                $jenis = MasterProvinsi::create([
                    'nama' => $request->nama,
                    'deskripsi' => $request->deskripsi,
                ]);
                Helper::createUserLog("Berhasil menambahkan provinsi ".$jenis->nama, auth()->user()->id, $this->title);
                DB::commit();
                return response()->json([
                    'status'=>200,
                ...
                DB::rollBack();
                Helper::createUserLog("Gagal menambah provinsi", auth()->user()->id, $this->title);
                return response()->json([
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                if ($data->nama==$request->nama) {
                    ...
                    Helper::createUserLog("Berhasil mengubah provinsi", auth()->user()->id, $this->title);
                    DB::commit();
                    return response()->json([
                        'status'=>200,
                    ...
                    $cek = MasterProvinsi::where('nama',$request->nama)->whereNot('id',$id)->first();
                    if ($cek) {
                        return response()->json([
                ...
                Helper::createUserLog("Gagal mengubah data provinsi", auth()->user()->id, $this->title);
                return response()->json([
                    'message'=>$e->getMessage()
                ],422);
            }
    ...
    public function destroy($id)
    {
        $data = MasterProvinsi::find($id);
        if ($data) {
            Helper::createUserLog("Berhasil menghapus provinsi ".$data->nama, auth()->user()->id, $this->title);
            $data->delete();
            return response()->json([
                'status'=>200,
            ...
            Helper::createUserLog("Gagal menghapus data provinsi", auth()->user()->id, $this->title);
            return response()->json([
